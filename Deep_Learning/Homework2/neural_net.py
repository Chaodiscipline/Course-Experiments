...
class ThreeLayerNet(object):
  ...
  def __init__(self, layers_dims):
    ...
    for l in range(1, L + 1):
        self.params['W' + str(l)] = np.random.randn(layers_dims[l-1],
                    layers_dims[l])*np.sqrt(2./layers_dims[l])
        self.params['b' + str(l)] = np.zeros((1, layers_dims[l]))
    

    print("W1 = " + str(self.params["W1"]))
    print("b1 = " + str(self.params["b1"]))
    print("W2 = " + str(self.params["W2"]))
    print("b2 = " + str(self.params["b2"]))
    print("W3 = " + str(self.params["W3"]))
    print("b3 = " + str(self.params["b3"]))
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    loss /= len(y)
    loss += 0.5 * reg * np.sum(W1 ** 2) + 0.5 * reg * np.sum(W2 ** 2) + 0.5 * reg * np.sum(W3 ** 2) # 正则化 
  ...
  def train(self, X, y, X_val, y_val, v, num_iters=100, batch_size=200,
...
#    print("test", V)
    ...
    for it in range(num_iters):
      ...
      shuffle_indexes = shuffle_indexes[0:batch_size-1]  # 从训练集中随机选取batch_size大小的数据 索引
      ...
      for l in range(L):
        ...
        # update parameters
        p["W" + str(l+1)] = p["W" + str(l+1)] + learning_rate * v["dW" + str(l+1)]
        p["b" + str(l+1)] = p["b" + str(l+1)] + learning_rate * v["db" + str(l+1)]
  ...
  def predict(self, X):
    ...
    layer2[layer2<0] = 0
    scores = np.dot(layer2, self.params['W3']) + self.params['b3']
